"""
Strategy — LongOnly Guard V2 Adaptive (INVERTED)

Same as LongOnlyGuardV2Adaptive but with INVERTED delta direction:
- When VIX is historically LOW (calm periods), RAISE thresholds (less reactive, avoid whipsaws)
- When VIX is historically HIGH (crisis periods), LOWER thresholds (more reactive, exit faster)

This is the OPPOSITE of the standard adaptive approach, based on empirical testing
showing that the inverted approach performs better on calm/normal market conditions.
"""
...
class LongOnlyGuardV2AdaptiveInverted(BaseStrategy):
    ...
    def __init__(
        ...
        vix_percentile_adapt: bool = False,
        vix_adapt_window:  int   = 252,
        vix_adapt_strength: float = 1.0,
    ):
        ...
        self.vix_percentile_adapt = vix_percentile_adapt
        self.vix_adapt_window  = vix_adapt_window
        self.vix_adapt_strength = vix_adapt_strength
    ...
    def _adapt_thresholds(self, vix_percentile: float) -> tuple:
        """
        INVERTED: Adjust vix_exit, vix_mid, vix_low, vix_reentry based on percentile.

        When VIX percentile is LOW (calm markets), RAISE thresholds (less reactive).
        When VIX percentile is HIGH (crisis), LOWER thresholds (more reactive).

        Returns: (vix_exit, vix_mid, vix_low, vix_reentry)
        """
        # INVERTED delta: multiply by -1
        delta = (vix_percentile - 0.5) * 2 * -1  # Note the -1 for inversion
